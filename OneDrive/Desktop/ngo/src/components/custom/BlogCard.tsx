...
const BlogCard = ({ src, linkCaption, title, titleClassName, className }: props) => {
  return (
    <>
      <div className={cn("w-[33.3%]  relative", className)}>
        <div className="relative h-[300px]">
          <Image src={src} alt="blog-pics" fill className="rounded-sm" />
        </div>
        <div className={cn("text-white bg-red-500 w-1/2 m-auto text-center p-2 absolute top-0 left-1/2 -translate-x-1/2 rounded-br-sm rounded-bl-sm md:text-sm text-xs font-semibold", titleClassName)}>
          {title}
        </div>
